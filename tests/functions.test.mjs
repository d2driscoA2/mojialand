...
test('grantPass: idempotent for pass, life, add, up', async () => {
  ...
  assert.equal(db.passes[0].ends_at, null); assert.equal(u1.code, null);
...
test('webhook: completed grants once and emails once; base64 body; refund; dispute', async () => {
  ...
  assert.equal(m.subject, 'Mojialand: your 48-hour pass code');
  ...
  assert.ok(m.text.includes('Keep this email. Your code works on up to 5 devices.'));
  ...
  assert.equal(db.emails.at(-1).subject, 'Mojialand: your Forever code');
});
